<template>
    <div class="flex flex-wrap justify-center max-w-md mx-auto">
        <h5 class="text-gray-700 mb-5 text-center text-2xl tracking-tight">Edit Wallet</h5>
        <form @submit.prevent="updateWallet" class="mb-5">
            <div class="flex flex-wrap justify-center">
                ...
                <button class="text-white bg-blue-600 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full px-5 py-2.5 text-center">Update Wallet</button>
            </div>
        </form>
        <button @click="deleteWallet" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full px-5 py-2.5 text-center">Delete Wallet</button>
    </div>
</template>
...
export default {
  ...
  data() {
      return {
          title: '',
          amount: '',
          uri: 'http://127.0.0.1:3525/api/wallets/' + this.id
      }
  },
  mounted() {
    fetch(this.uri)
